'''
  This script is a modified copy of the script written by last year's team to record footage, generate AES keys, and encrypt footage.
  Before, the script used AES (EAX mode) to encrypt each video, and an RSA pubkey to encrypt each AES key and video. The original script
  also intended for 'dumping' all the videos sometime after recording them.
  
  Since we'd like to use the drone for getting footage as soon as possible when heading to the JACart, this script is currently designed 
  to take 4 second videos from the GoPro, download each file, and wait 4 seconds between each recording so the camera/API can
  ...
  As of 3/28/2023, lines that have to do with drone location are commented out, since the drone is having flight issues, and we
  need to test these scripts with another RPi.
'''
...
def record_video(sleep=2):
    ...
    try:
        gopro.downloadLowRes(custom_filename='videos/'+str(random.randint(0, 1000))+'.mp4')
        gopro.delete("all")
